...
const NB_SUCCESS_FOR_WINNING = 6; // nombre de points pour gagner
...
async function generateResponse(success, startTime, getCardFunction, lang, jlptGrade, kanjis_joues, correctIndexes, currentCardIndex) {
    ...
    if (currentCardIndex == 0 || (correctIndexes.length - currentCardIndex) < NB_MIN_CARDS) {
        ...
        // pour chacune de nos cards, on met à jour notre liste de kanji joués
        kanjis_joues = kanjis_joues.concat(cards.cards.map(c => ({
            kanji: c.proposal,
            correct: false // on initialise à faux, on mettra à vrai si le joueur a trouvé
        })));
    ...
    const gameToken = generateGameToken(encryptPayload({ correctIndexes: correctIndexes, success, startTime, jlptGrade, kanjis_joues, currentCardIndex }));
...
const _getCards = async (getCardFunction, correctIndexes, lang, jlptGrade) => {
    ...
    return { cards, kanji: kanji_a_deviner };
}
        ...
        try {
            ...
            if (choiceIndex === payload.correctIndexes[payload.currentCardIndex]) {
                ...
                if (payload.kanjis_joues && payload.kanjis_joues.length > 0) {
                    payload.kanjis_joues[payload.kanjis_joues.length - 1].correct = correct;
                }
            }
            ...
            if (payload.success >= NB_SUCCESS_FOR_WINNING) {
                ...
                if (userId && Model) {
                    ...
                    if (payload.kanjis_joues) {
                        try {
                            await enregistrerProgressions(userId, payload.kanjis_joues);
                        } catch (err) {
            ...
            // on affiche nos correctIndexes DANS LA CONSOLE AVEC LES INDEX DU TABLEAU
            //console.log(payload.correctIndexes);
            //console.log("tour num : ", payload.currentCardIndex, "Votre Réponse :", choiceIndex, "Index de la réponse :", payload.correctIndexes[payload.currentCardIndex]);
